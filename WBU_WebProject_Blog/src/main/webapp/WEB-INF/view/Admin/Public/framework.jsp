<%@ page language="java" contentType="text/html; charset=UTF-8"
         pageEncoding="UTF-8" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
<%@ taglib prefix="rapid" uri="http://www.rapid-framework.org.cn/rapid" %>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="utf-8">
    <!-- 上述3个meta标签*必须*放在最前面，任何其他内容都*必须*跟随其后！ -->
    <link rel="shortcut icon" href="/img/logo.png">
    <title>
        ${options.optionSiteTitle}后台
            <rapid:block name="title"></rapid:block>
    </title>
    <link rel="stylesheet" href="/plugin/mtc/css/mtc.css">
    <link rel="stylesheet" href="/css/back.css">
    <link rel="stylesheet" href="/plugin/font-awesome/css/font-awesome.min.css">
    <rapid:block name="header-style"></rapid:block>
    <rapid:block name="header-script"></rapid:block>
</head>
<body>
<div class="mtc-layout mtc-layout-admin">
    <div class="mtc-header">
        <div class="mtc-logo"><a href="/admin" style="color:#009688;">
        ${options.optionSiteTitle}后台
        </a>
        </div>
        <!-- 头部区域（可配合mtc已有的水平导航） -->
        <ul class="mtc-nav mtc-layout-left">
            <li class="mtc-nav-item"><a href="/" target="_blank">前台</a></li>
            <li class="mtc-nav-item">
                <a href="javascript:;">新建</a>
                <dl class="mtc-nav-child">
                    <dd><a href="/admin/article/insert">文章</a></dd>
                    <dd><a href="/admin/page/insert">页面</a></dd>
                    <dd><a href="/admin/category/insert">分类</a></dd>
                    <dd><a href="/admin/notice/insert">公告</a></dd>
                    <dd><a href="/admin/link/insert">链接</a></dd>
                </dl>
            </li>
        </ul>
        <ul class="mtc-nav mtc-layout-right">
            <li class="mtc-nav-item">
                <a href="javascript:;">
                    <img src="${sessionScope.user.userAvatar}" class="mtc-nav-img">
                    ${sessionScope.user.userName}
                </a>
                <dl class="mtc-nav-child">
                    <dd><a href="/admin/profile">基本资料</a></dd>
                </dl>
            </li>
            <li class="mtc-nav-item">
                <a href="/admin/logout">退了</a>
            </li>
        </ul>
    </div>

    <div class="mtc-side mtc-bg-black">
        <div class="mtc-side-scroll">
            <!-- 左侧导航区域（可配合mtc已有的垂直导航） -->

            <c:if test="${sessionScope.user != null && sessionScope.user.userRole == 'admin'}">
                <ul class="mtc-nav mtc-nav-tree"  lay-filter="test">
                <li class="mtc-nav-item mtc-nav-itemed">
                    <a class="" href="javascript:;">文章</a>
                    <dl class="mtc-nav-child">
                        <dd><a href="/admin/article">全部文章</a></dd>
                        <dd><a href="/admin/article/insert">写文章</a></dd>
                        <dd><a href="/admin/category">全部分类</a></dd>
                        <dd><a href="/admin/tag">全部标签</a></dd>
                    </dl>
                </li>
                <li class="mtc-nav-item">
                    <a href="javascript:;">页面</a>
                    <dl class="mtc-nav-child">
                        <dd><a href="/admin/page">全部页面</a></dd>
                        <dd><a href="/admin/page/insert">添加页面</a></dd>
                    </dl>
                </li>
                <li class="mtc-nav-item">
                    <a class="" href="javascript:;">
                        链接
                    </a>
                    <dl class="mtc-nav-child">
                        <dd><a href="/admin/link">全部链接</a></dd>
                        <dd><a href="/admin/link/insert">添加链接</a></dd>
                    </dl>
                </li>
                <li class="mtc-nav-item">
                    <a href="javascript:;">公告</a>
                    <dl class="mtc-nav-child">
                        <dd><a href="/admin/notice">全部公告</a></dd>
                        <dd><a href="/admin/notice/insert">添加公告</a></dd>
                    </dl>
                </li>
                <li class="mtc-nav-item">
                    <a href="/admin/comment">
                        评论
                    </a>
                </li>
                <li class="mtc-nav-item">
                    <a href="javascript:;">用户</a>
                    <dl class="mtc-nav-child">
                        <dd><a href="/admin/user">全部用户</a></dd>
                        <dd><a href="/admin/user/insert">添加用户</a></dd>
                    </dl>
                </li>
                <li class="mtc-nav-item">
                    <a href="javascript:;">设置</a>
                    <dl class="mtc-nav-child">
                        <dd><a href="/admin/menu">菜单</a></dd>
                        <dd><a href="/admin/options">主要选项</a></dd>
                    </dl>
                </li>
            </ul>
            </c:if>

            <c:if test="${sessionScope.user != null && sessionScope.user.userRole == 'user'}">
                <ul class="mtc-nav mtc-nav-tree"  lay-filter="test">
                    <li class="mtc-nav-item mtc-nav-itemed">
                        <a class="" href="javascript:;">文章</a>
                        <dl class="mtc-nav-child">
                            <dd><a href="/admin/article">我的文章</a></dd>
                            <dd><a href="/admin/article/insert">写文章</a></dd>
                        </dl>
                    </li>
                    <li class="mtc-nav-item mtc-nav-itemed">
                        <a class="" href="javascript:;">评论</a>
                        <dl class="mtc-nav-child">
                            <dd><a href="/admin/comment">我的评论</a></dd>
                            <dd><a href="/admin/comment/receive">评论我的</a></dd>
                        </dl>
                    </li>
                </ul>
            </c:if>
        </div>
    </div>

    <div class="mtc-body">
        <!-- 内容主体区域 -->
        <div style="padding: 15px;">
            <rapid:block name="content">

            </rapid:block>
        </div>
    </div>

    <div class="mtc-footer">
        <!-- 底部固定区域 -->
        © <a href="http://blog.Mingtc.com">言曌博客</a> 2017  欢迎加入开发者交流群 590480292，博主免费回答大家日常问题。同时博主代做毕设，解决开发问题，详情参看 <a href="https://Mingtc.com/shop.html" target="_blank">有偿服务</a>
    </div>
</div>

<script src="/js/jquery.min.js"></script>
<script src="/plugin/mtc/mtc.all.js"></script>
<script src="/js/back.js"></script>
<rapid:block name="footer-script">

</rapid:block>
<script>
    //给文本编辑器的iframe引入代码高亮的css
    $("iframe").contents().find("head").append("<link rel=\"stylesheet\" href=\"/css/highlight.css\">\n");

</script>

</body>
</html>
